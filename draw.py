...
tf.compat.v1.flags.DEFINE_boolean("read_attn", True, "enable attention for reader")
tf.compat.v1.flags.DEFINE_boolean("write_attn",True, "enable attention for writer")
...
lstm_enc = tf.compat.v1.nn.rnn_cell.LSTMCell(enc_size, state_is_tuple=True) # encoder Op
lstm_dec = tf.compat.v1.nn.rnn_cell.LSTMCell(dec_size, state_is_tuple=True) # decoder Op
...
# train_data = tf.examples.tutorials.mnist.input_data.read_data_sets(data_directory, one_hot=True).train # binarized (0-1) mnist data
train_data = tfds.load("binarized_mnist", data_dir=data_directory)['train'].batch(batch_size=batch_size) # binarized (0-1) mnist data
