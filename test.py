...
z_size=40#10 # QSampler output size
T=40#10 # generation sequence length
    ...
    def load_images_from_folder(self,folder,max_img_counnter):
        ...
        for filename in os.listdir(folder):
            ...
            if img is not None:
                ...
                image_resized = resize(gray_image, (A,B),
                       anti_aliasing=True)
...
# saver.restore(sess, "models/drawmodel9000.ckpt") # to restore from model, uncomment this line

for i in range(train_iters):
    # xtrain,_=train_data.next_batch(batch_size) # xtrain is (batch_size x img_size)
    xtrain=train_data.next_batch()
    ...
    if i%1000==0:
        print("iter=%d : Lx: %f Lz: %f" % (i,Lxs[i],Lzs[i]))
        ckpt_file=os.path.join(FLAGS.data_dir,"models/drawmodel{0}.ckpt".format(i))
